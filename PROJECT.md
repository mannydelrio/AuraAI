...
| Always-on overlay + voice | ✅ | both services `START_STICKY` + auto-start in MainActivity.onCreate + BootReceiver (Portal kills bg process on app-switch; services self-restart). Bubble stays above every app; mic listener restarts automatically. |
